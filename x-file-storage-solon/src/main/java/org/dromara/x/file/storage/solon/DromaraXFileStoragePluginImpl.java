  ...
  public void start(final AppContext context) {
    ...
    context.getBeanAsync(SolonFileStorageProperties.class, cfg -> {
      ...
      for (final SolonFileStorageProperties.SpringLocalConfig localConfig : localList) {
        ...
        log.info("[x-file-storage] [local] 添加本地存储映射: {} -> {}", localConfig.getPathPatterns(), localConfig.getBasePath());
      }
      ...
      for (final SolonFileStorageProperties.SpringLocalPlusConfig localPlusConfig : localPlusList) {
        ...
        //添加本地绝对目录（例：/img/logo.jpg 映射地址为：/data/sss/app/img/logo.jpg）
        StaticMappings.add(localPlusConfig.getPathPatterns(), new FileStaticRepository(localPlusConfig.getStoragePath()));
        log.info("[x-file-storage] [localPlus] 添加本地存储映射: {} -> {}", localPlusConfig.getPathPatterns(), localPlusConfig.getStoragePath());
      }
