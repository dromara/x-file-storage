...
amazon-s3: # Amazon S3
  - platform: amazon-s3-1 # 存储平台标识
    ...
    path-style-access: false # 路径风格访问，默认 false；接入 RustFS / Ceph RGW 等自建 S3 兼容服务时设为 true
    chunked-encoding: true # 分块传输编码，默认 true；接入 Cloudflare R2 等部分 S3 兼容服务时设为 false
    accelerate: false # S3 Transfer Acceleration（传输加速），仅 AWS S3 真身且桶上已开启加速时设为 true
    bucket-name: abcd #桶名称
    domain: https://abcd.s3.ap-east-1.amazonaws.com/ # 访问域名，注意“/”结尾，例如：https://abcd.s3.ap-east-1.amazonaws.com/
    base-path: s3/ # 基础路径
```

接入 RustFS / Ceph RGW 等自建 S3 参考以下使用方式（路径风格访问）

```yaml
amazon-s3:
  - platform: rustfs-1 # 存储平台标识
    enable-storage: true  # 启用存储
    access-key: ??
    secret-key: ??
    path-style-access: true # 路径风格访问，默认 false；接入 RustFS / Ceph RGW 等自建 S3 兼容服务时设为 true
    region: ap-east-1 # 随便写一个
    end-point: http://192.168.3.250:9000/ # 必填
    bucket-name: abc # 桶名称
    domain: http://192.168.3.250:9000/abc/ # 访问域名，注意“/”结尾，例如：https://abc.hn-bkt.clouddn.com/
```
...
amazon-s3-v2: # Amazon S3 V2
  - platform: amazon-s3-v2-1 # 存储平台标识
    ...
    chunked-encoding: true # 分块传输编码，默认 true；接入 Cloudflare R2 等部分 S3 兼容服务时设为 false
    accelerate: false # S3 Transfer Acceleration（传输加速），仅 AWS S3 真身且桶上已开启加速时设为 true
    request-checksum-calculation: # 请求校验和计算策略，留空走 SDK 默认（WHEN_SUPPORTED）；接入旧版本 RustFS / Garage / SeaweedFS 等尚未实现 STREAMING-UNSIGNED-PAYLOAD-TRAILER 的 S3 兼容服务时设为 WHEN_REQUIRED
    bucket-name: abcd #桶名称
    domain: https://abcd.s3.ap-east-1.amazonaws.com/ # 访问域名，注意“/”结尾，例如：https://abcd.s3.ap-east-1.amazonaws.com/
    base-path: s3/ # 基础路径
```
...
amazon-s3-v2:
  - platform: rustfs-1 # 存储平台标识
    ...
    secret-key: ??
    path-style-access: true # 路径风格访问，默认 false；接入 RustFS / Ceph RGW 等自建 S3 兼容服务时设为 true
    request-checksum-calculation: WHEN_REQUIRED # AWS SDK for Java 2.x 自 2.30.0 起默认会发送 STREAMING-UNSIGNED-PAYLOAD-TRAILER，旧版本 RustFS 等尚未实现该特性时需设为 WHEN_REQUIRED
    region: ap-east-1 # 随便写一个
    end-point: http://192.168.3.250:9000/ # 必填
    bucket-name: abc # 桶名称
    domain: http://192.168.3.250:9000/abc/ # 访问域名，注意“/”结尾，例如：https://abc.hn-bkt.clouddn.com/
...
更多参数请参考 `org.dromara.x.file.storage.spring.SpringFileStorageProperties.SpringAmazonS3V2Config`
...
go-fastdfs:
  - platform: go-fastdfs-1 # 存储平台标识
    ...
    ramdom: true
    base-path: '' # 基础路径
    auth-token: '' # 自定义认证 token，留空则不发送，仅当服务端配置 AuthUrl 鉴权时需要填写，将作为 HTTP 请求头 Auth-Token 发送
```
