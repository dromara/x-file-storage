...
## 📦2.4.0 :id=_240
2026-05-06
- Amazon S3 / Amazon S3 V2 新增 path-style-access、chunked-encoding、accelerate 配置项，支持对接 RustFS、Ceph RGW、Cloudflare R2 等 S3 兼容存储服务及 AWS 传输加速
- Amazon S3 V2 新增 request-checksum-calculation 配置项，并升级 `software.amazon.awssdk:s3` 到 2.44.3，解决 SDK 自 2.30.0 起默认计算 CRC32 校验和并发送 STREAMING-UNSIGNED-PAYLOAD-TRAILER 导致 RustFS 等部分 S3 兼容存储返回 `invalid header: x-amz-content-sha256` 的问题（参考 [Issue #338](https://github.com/dromara/x-file-storage/issues/338)）
- 修复 阿里云 OSS 在进程触发过 OOM 等场景下内部 Apache HttpClient 连接池被永久 shutdown 导致后续请求全部失败的问题，新增 `RefreshableClient` 包装实现自动自愈机制（参考 [Issue #331](https://github.com/dromara/x-file-storage/issues/331)）
- go-fastdfs 新增 `auth-token` 配置项，对接服务端开启 `AuthUrl` 自定义认证时会作为 HTTP 请求头 `Auth-Token` 发送；同时修正 `GoFastDfsFileStorageClientFactory` 类注释笔误（参考 [Issue #333](https://github.com/dromara/x-file-storage/issues/333)）
- 优化本地存储平台文件删除逻辑，避免未传 filename 时删除整个目录
