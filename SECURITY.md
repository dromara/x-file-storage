...
Thanks for helping keep x-file-storage and the wider [dromara](https://github.com/dromara) ecosystem secure.
...
1. **GitHub's Private Vulnerability Reporting (preferred).**
   Open a private security advisory at <https://github.com/dromara/x-file-storage/security/advisories/new>.
...
x-file-storage is a unified Java file-storage / file-upload abstraction with adapters for local disk, S3-compatible object storage, FTP/SFTP, WebDAV, and Chinese cloud providers. The maintainers support security fixes on the latest minor release. Older releases will not generally receive backports.

In-scope vulnerability classes include:

- Path traversal in local-disk / FTP / SFTP / WebDAV adapters
- SSRF via remote-fetch / signed-URL / pre-signed-URL handling
- Cross-bucket / cross-prefix access control flaws
- Insecure deserialization in adapter response parsing
- XXE / SSRF in S3-compatible XML response parsing
- Cryptographic misuse in stored secrets / pre-signed URL signing
- Content-type / file-type bypass that enables stored XSS or RCE

Out-of-scope:

- Findings against intentionally trusted-admin features (config that the framework explicitly delegates to the integrator).
- Issues that require an attacker to already have full bucket / disk access.
...
Reporters who have helped harden x-file-storage via responsible disclosure will be listed here once the corresponding advisory is published.
